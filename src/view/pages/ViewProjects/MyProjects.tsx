...
import { useToast } from "../../common/Toast/ToastProvider.tsx";
...
const MyProjects = () => {
  ...
  useEffect(() => {
    const checkAuthentication = () => {
      ...
      if (loginTime && isSessionExpired(parseInt(loginTime))) {
        ["token", "refreshToken", "username", "role", "loginTime"].forEach(
          (key) => localStorage.removeItem(key),
        );
        toast.show({
          message: `Your session has expired after ${SESSION_DURATION_HOURS} hours. Please log in again.`,
          type: "error",
        });
        navigate("/login");
        return;
    ...
    const fetchMyProjects = async () => {
      ...
      try {
        ...
        const response = await backendApi.get(
          `/project/user/${encodeURIComponent(currentUser.email)}`,
          {
        ...
        if (error.response?.status === 401) {
          ...
          setError(
            `Failed to load projects: ${error.response?.data?.error || error.message}`,
          );
        }
      } finally {
        setLoading(false);
  ...
  const handleDeleteProject = async (projectId: string) => {
    ...
    try {
      ...
      setProjects(
        projects.filter((p) => p.id !== projectId && p._id !== projectId),
      );

      if (
        selectedProject &&
        (selectedProject.id === projectId || selectedProject._id === projectId)
      ) {
        setShowModal(false);
      ...
      toast.show({ message: "Project deleted successfully!", type: "success" });
    } catch (error: any) {
      if (error.response?.status === 401) {
        toast.show({
          message: "Session expired. Please log in again.",
  ...
  const removeArrayItem = (field: "materials" | "steps", index: number) => {
    ...
    setEditingProject({
      ...editingProject,
      [field]: editingProject[field].filter((_, i) => i !== index),
    });
  };

  const handleLogout = () => {
    ["token", "refreshToken", "username", "role", "loginTime"].forEach((key) =>
      localStorage.removeItem(key),
    );
    toast.show({ message: "Successfully logged out!", type: "success" });
    navigate("/login");
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-blue-50 p-4">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          className="absolute top-20 left-10 text-pink-500/20 animate-bounce"
          style={{ animationDelay: "0s", animationDuration: "3s" }}
        >
          <Scissors className="w-16 h-16" />
        </div>
        <div
          className="absolute top-40 right-20 text-purple-500/20 animate-bounce"
          style={{ animationDelay: "1s", animationDuration: "4s" }}
        >
          <PaintBucket className="w-20 h-20" />
        </div>
        <div
          className="absolute bottom-32 left-1/4 text-blue-500/20 animate-bounce"
          style={{ animationDelay: "2s", animationDuration: "3.5s" }}
        >
          <Palette className="w-14 h-14" />
        </div>
        <div
          className="absolute top-1/3 right-1/4 text-pink-500/20 animate-bounce"
          style={{ animationDelay: "1.5s", animationDuration: "3.8s" }}
        >
          <Sparkles className="w-12 h-12" />
        </div>
        <div
          className="absolute bottom-20 right-1/3 text-purple-500/20 animate-bounce"
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 bg-white/80 backdrop-blur-lg rounded-3xl p-8 border border-white/40 shadow-xl">
          ...
          <p className="font-bold text-gray-800 text-xl">
            Hi,{" "}
            {currentUser.username ? currentUser.username.split(" ")[0] : "User"}
            !
          </p>{" "}
        </div>
            ...
            {projects.length === 0 ? (
              <div className="bg-white/80 backdrop-blur-lg rounded-3xl p-16 border border-white/40 text-center shadow-xl">
                ...
                <p className="text-gray-600 mb-8 text-sm max-w-md mx-auto leading-relaxed">
                  You haven't uploaded any projects yet. Start creating and
                  sharing your amazing ideas with the community!
                </p>
                <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                  {projects.map((project) => (
                    <div
                      ...
                      className="bg-white/90 backdrop-blur-lg rounded-3xl shadow-xl border border-white/50 overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 group"
                    >
                      <div className="h-52 bg-gradient-to-br from-pink-100 via-purple-100 to-blue-100 flex items-center justify-center relative overflow-hidden">
                        {project.imageUrl ? (
                          ...
                          <div className="text-gray-400 text-6xl group-hover:scale-110 transition-transform duration-500">
                            🖼️
                          </div>
                        )}
            ...
            <div className="bg-white/95 backdrop-blur-lg rounded-3xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden border border-white/50">
              ...
              <div className="p-8 overflow-y-auto max-h-[calc(90vh-140px)]">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div>
                    <h3 className="text-xl font-bold text-gray-800 mb-4 bg-gradient-to-r from-pink-500 to-purple-500 bg-clip-text text-transparent">
                      Project Info
                    </h3>
                    ...
                    <h4 className="text-lg font-bold text-gray-800 mb-4 bg-gradient-to-r from-pink-500 to-purple-500 bg-clip-text text-transparent">
                      Description
                    </h4>
                    {isEditMode ? (
                      <textarea
                      ...
                      <p className="text-gray-600 text-base leading-relaxed bg-gray-50 p-4 rounded-2xl">
                      ...
                      <div className="bg-blue-50 p-4 rounded-2xl">
                        {editingProject.steps.map((step, index) => (
                          <div key={index} className="flex gap-2 mb-3">
                            <span className="text-sm font-semibold text-gray-600 mt-2 min-w-[20px]">
                              {index + 1}.
                            </span>
                            <textarea
                              value={step}
