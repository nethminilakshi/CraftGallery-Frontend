...
import {
  ...
  clearError,
  testEmail,
} from "../../../slices/projectUploadSlice";
...
const ProjectUploadForm = () => {
  ...
  const {
    ...
    (state: RootState) => state.categories || state.projectUpload || {},
  ) as {
  ...
  useEffect(() => {
    const checkAuthentication = () => {
      ...
      if (!token) {
        toast.show({
          message: "Please log in to upload projects",
          type: "error",
        });
        navigate("/login");
        return;
  ...
  const handleSubmit = async () => {
    ...
    if (validationError) {
      toast.show({ message: validationError, type: "error" });
      return;
    }
  ...
  const handleTestEmail = async () => {
    if (!testEmailAddress) {
      toast.show({ message: "Please enter an email address", type: "error" });
      return;
    }
    ...
    try {
      ...
      toast.show({ message: "Failed to send test email", type: "error" });
    }
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 text-sm">
                  <Mail size={16} />
                  <span
                    className={emailSent ? "text-green-100" : "text-yellow-200"}
                  >
                    {emailSent
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <div>
                      <p className="font-bold text-white text-lg">
                        Welcome, {currentUser.username || "Creator"}!
                      </p>
                    </div>
                  </div>
                  {/*{currentUser.exp && (*/}
            ...
            <div className="group">
              ...
              >
                ...
                {!categoriesLoading &&
                  ...
                  categories &&
                  categories.map(
                    (categoryObj: CategoryData | string, index: number) => (
                      <option
                        key={index}
                        value={
              ...
              {formData.materials.map((material: string, index: number) => (
                <div key={index} className="flex gap-3 mb-3">
                  <input
                    value={material}
                    onChange={(e) =>
                      updateArray(index, e.target.value, "materials")
                    }
              ...
              {formData.steps.map((step: string, index: number) => (
                <div key={index} className="flex gap-3 mb-4">
                  ...
                  <textarea
                    value={step}
                    onChange={(e) =>
                      updateArray(index, e.target.value, "steps")
                    }
